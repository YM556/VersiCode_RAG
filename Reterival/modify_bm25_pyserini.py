...
def modify_single_dataset(dataset_dir, output_metadir):
    input_path = os.path.join(dataset_dir, "corpus.jsonl")
    output_dir = os.path.join(output_metadir, "corpus")
...
def index_single_dataset(output_metadir, index_dir, dataset):
    input_dir = os.path.join(output_metadir, "corpus")
    output_dir = os.path.join(index_dir, "corpus")
...
def search_single_dataset(index_dir, dataset="versicode", top_k=10, k1=1.5, b=0.75):
...
if __name__ == '__main__':
    ...
    all_datasets = ["humaneval", "mbpp",
                    "odex_en", "ds1000_all_completion", "repoeval/function",
                    "livecodebench", "code_search_net_python",
                    "versicode_python/stackoverflow/stackoverflow_line",
                    "versicode_python/downstream_application_code/downstream_application_code_block",
                    "versicode_python/library_source_code/library_source_code_block"]
    parser = argparse.ArgumentParser()
    parser.add_argument('--dataset', type=str, default="all",
                        choices=all_datasets+["all"])
    parser.add_argument('--dataset_dir', type=str, default='datasets')
    parser.add_argument('--output_metadir', type=str, default='/root/autodl-tmp/VersiCode-RAG/Reterival/output')
    parser.add_argument('--index_dir', type=str, default='/root/autodl-tmp/VersiCode-RAG/Reterival/pyserini')
    ...
    if args.stage == 'preprocess' or args.stage == 'all':
        for dataset in datasets:
            ...
            modify_single_dataset(args.dataset_dir, args.output_metadir)
