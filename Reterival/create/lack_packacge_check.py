...
parser.add_argument("--dir",help="The target file directory",default="/Volumes/kjz-SSD/Datasets/VersiCode_Raw/VersiCode_Raw/library_source_code/corpus_jsonl/")
parser.add_argument("--output_dir",default="/Volumes/kjz-SSD/Datasets/VersiCode_Raw/VersiCode_Raw/library_source_code/corpus_jsonl/")
parser.add_argument("--postfix",default=".jsonl")
args = parser.parse_args()

library_list = []
def main():
    for filename in tqdm(os.listdir(args.dir),total=len(os.listdir(args.dir))):
        if filename=="streamlit.jsonl" or filename == "tensorflow.jsonl":
            # 读取 JSONL 文件并处理数据
