...
parser.add_argument("--corpus_dir",default="VersiCode_Raw/VersiCode_Raw/downstream_application_code/project_specific_version_corpus",help="The directory contains all the library source code")
...
def reterive_msg(corpus_dir,dependency,text,version) -> list:
    ...
    try:
        ...
        if os.path.exists(filepath):
            ...
            _source_code = create_collection(corpus_dir,dependency,version,text,1)
...
def build_samples(sample_source:str,sample_num):
    ...
    if sample_source in sources:
        #using downstream_application_code as
...
for idx,data in enumerate(data_list[:sample_num]):
    ...
    try:
        ...
        if with_RAG=="True":
            # import ipdb
            # ipdb.set_trace()
            source_code,retrieve_time = reterive_msg(corpus_dir, data['dependency'], description,data["version"][2:])
...
if not os.path.exists(save_folder_path):
    os.makedirs(save_folder_path)
save_json_path = os.path.join(save_folder_path, "token_rag_downstream_application.jsonl")
