...
parser.add_argument("--task",help="new_to_old or old_to new",default="new_to_old")
...
json_path = os.path.join(prefix_path,'VersiCode/data/VersiCode_Benchmark/VersiCode_Benchmark/code_editing','code_editing_{}.json'.format(args.task))
...
def compute_pass_at_k(predictions, expected_output,k):
    return any(predictions) == expected_output
...
if args.task == "old_to_new":
    _answer_code = "new_code"
    _answer_name = "new_name"
else:
    _answer_code = "old_code"
    _answer_name = "old_name"
...
for data in data_list[:sample_num]:
    ...
    if with_RAG==False:
        ...
        try:
            ...
            answer_code = data[_answer_code]
            answer_name = data[_answer_name]
