...
def retrieve_msg(description_value,dependency,version):
    ...
    with driver.session() as session:
        ...
        for record in results:
            ...
            # a corresponds to the below one
            ebd_str = record['a']["embedding"]
            ebd_lst = list(map(float,ebd_str.split(",")))
            # 将embedding转换为NumPy数组
            embeddings.append(np.array(ebd_lst))
        ...
        for idx,record in enumerate(results):
            if idx in top_indices:
                closest_node_ids.append(record['a']["api"])

        # 处理结果
        triples = []
        for api in closest_node_ids:
            api = api.split("/")[-1]
...
def build_samples(sample_source:str,sample_num):
    ...
    if sample_source in sources:
        sample_path = os.path.join(prefix_path,'VersiCode/data/VersiCode_Benchmark/VersiCode_Benchmark/code_completion/',sample_source,'{}_block.json'.format(sample_source))
        ...
        for source in sources:
            sample_paths.append(os.path.join(prefix_path,'VersiCode/data/VersiCode_Benchmark/VersiCode_Benchmark/code_completion/',source, '{}_block.json'.format(source)))
...
save_folder_path = "./tmp/block_results/"
if not os.path.exists(save_folder_path):
    os.makedirs(save_folder_path)
save_json_path = os.path.join(save_folder_path, "test_block_lib_graph.jsonl")
